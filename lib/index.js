...
//从postcss.process方法中传递的opts.from中得到当前处理文件路径
function _getCurrentProcessFilePath(from) {
  var tmp = from.split(path.sep);
  tmp.pop();
...
var cssprocess = function(css, opts) {
  //保存当前处理文件路径
  _currentProcessFilePath = _getCurrentProcessFilePath(opts.from);
